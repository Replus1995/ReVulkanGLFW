...
void VulkanGLFWApp::createInstance(const char * pApplicationName, const char * pEngineName)
{
...
void VulkanGLFWApp::setupDebugCallback()
{
	...
	if (CreateDebugReportCallbackEXT(m_Instance, &createInfo, nullptr, &debug_callback) != VK_SUCCESS) {
		throw std::runtime_error("failed to set up debug callback!");
	}

}

void VulkanGLFWApp::pickPhysicalDevice()
{
...
VulkanGLFWApp::QueueFamilyIndices VulkanGLFWApp::findQueueFamilies(VkPhysicalDevice device)
{
...
bool VulkanGLFWApp::checkDeviceExtensionSupport(VkPhysicalDevice device)
{
...
bool VulkanGLFWApp::isDeviceSuitable(VkPhysicalDevice device)
{
	QueueFamilyIndices indices = findQueueFamilies(device);
	if (indices.GraphicsFamilyIndex < 0 || indices.PresentFamilyIndex < 0) {
		return false;
	}
...
void VulkanGLFWApp::createLogicalDevice()
{
	...
	std::set<int> uniqueQueueFamilies = { family_indices.GraphicsFamilyIndex, family_indices.PresentFamilyIndex };
...
VulkanGLFWApp::SwapChainSupportDetails VulkanGLFWApp::querySwapChainSupport(VkPhysicalDevice device)
{
...
VkSurfaceFormatKHR VulkanGLFWApp::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats)
{
...
VkPresentModeKHR VulkanGLFWApp::chooseSwapPresentMode(const std::vector<VkPresentModeKHR> availablePresentModes)
{
...
VkExtent2D VulkanGLFWApp::chooseSwapExtent(const VkSurfaceCapabilitiesKHR & capabilities)
{
...
void VulkanGLFWApp::createSwapChain()
{
...
void VulkanGLFWApp::createImageViews()
{
...
void VulkanGLFWApp::createRenderPass()
{
...
void VulkanGLFWApp::createGraphicsPipeline()
{
	...
	std::vector<char> vertShaderCode = readFile("./shader/shader_vert.spv");
	std::vector<char> fragShaderCode = readFile("./shader/shader_frag.spv");
...
void VulkanGLFWApp::createFramebuffers()
{
...
void VulkanGLFWApp::createCommandPool()
{
...
void VulkanGLFWApp::createIndexBuffer()
{
...
void VulkanGLFWApp::createUniformBuffer()
{
...
uint32_t VulkanGLFWApp::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
...
void VulkanGLFWApp::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer & buffer, VkDeviceMemory & bufferMemory)
{
...
void VulkanGLFWApp::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size)
{
...
VkCommandBuffer VulkanGLFWApp::beginSingleTimeCommands()
{
	VkCommandBufferAllocateInfo allocInfo = {};
	allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
	allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
...
void VulkanGLFWApp::endSingleTimeCommands(VkCommandBuffer commandBuffer)
{
...
void VulkanGLFWApp::createDescriptorSetLayout()
{
...
void VulkanGLFWApp::createDescriptorPool()
{
...
void VulkanGLFWApp::createDescriptorSet()
{
...
void VulkanGLFWApp::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout)
{
	VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void VulkanGLFWApp::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height)
{
	VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
std::vector<const char*> VulkanGLFWApp::getRequiredExtensions()
{
